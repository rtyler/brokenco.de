...
AWS account is almost above $1.50, so my budget is set to where if somehow this
starts costing me more than a couple of dollars a month, AWS will email me so I
can figure out what to save my snapple money.
...
immediately identified by robots around the world, most of them probably
malicious in intent, but a few designed to help developers who made a mistake.
...
on the account and still recommended:
...
accordingly. There is one incidence of leaking AWS keys on GitHub that I am
personally aware of (friend of a friend, I swear!) where an errant `git add`
resulted in a local credentials file being pushed to a personal sandbox
(public) repository. Because the email account linked to the AWS account was
not regularly checked, the key was used abusively to rack up a few hundred
dollars on an AWS bill before the keys were revoked.
